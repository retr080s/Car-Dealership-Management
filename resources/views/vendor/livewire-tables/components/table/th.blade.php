...
@if ($component->isTailwind())
    <th scope="col" {{
        $attributes->merge($customAttributes)
            ->class(['px-6 py-3 text-left text-xs font-medium whitespace-nowrap text-gray-500 uppercase tracking-wider dark:bg-teal-700/50 dark:text-white' => $customAttributes['default'] ?? true])
                ...
                <button
                    ...
                    {{
                        $attributes->merge($customSortButtonAttributes)
                            ->class(['flex items-center space-x-1 text-left text-xs leading-4 font-medium text-gray-200 uppercase tracking-wider group focus:outline-none dark:text-gray-200' => $customSortButtonAttributes['default'] ?? true])
